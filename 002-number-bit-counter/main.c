...
static uint8_t  get_bit_count_from_input_number (int32_t input_value); 
...
int main(void)
{
    ...
    scanf("%d",&input_value);
    printf(" Input value: %d, it is compounded by: %d bit(s).\r\n",input_value,get_bit_count_from_input_number(input_value));
    return 0;
}
...
uint8_t get_bit_count_from_input_number (int32_t input_value)
{
    u_int8_t  number_of_bits=0ul;
